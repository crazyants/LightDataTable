    ...
    internal static class DbSchema
    {
        ...
        internal static long SaveObject(ICustomRepository repository, InterFace.IDbEntity o, bool isIndependentData = false, bool commit = false)
        {
            ...
            try
            {
                ...
                var cols = props.Where(x => availableColumns.FindByPrimaryKey<bool>(x.GetPropertyName()) && (!x.PropertyType.IsClass || x.PropertyType == typeof(string)) && x.GetCustomAttribute<ExcludeFromAbstract>() == null && x.GetCustomAttribute<PrimaryKey>() == null);
                ...
                foreach (var prop in props.Where(x => x.PropertyType.IsClass && x.PropertyType != typeof(string) && x.GetCustomAttribute<ExcludeFromAbstract>() == null))
                {
                    var independentData = prop.GetCustomAttribute<IndependentData>() != null;
                    var type = prop.PropertyType;
                    if (prop.PropertyType.IsGenericType && prop.PropertyType.GetGenericTypeDefinition()?.Name == "List`1" && prop.PropertyType.GenericTypeArguments.Length > 0)
                    ...
                    if (oValue is IList)
                    {
                        ...
                        {
                            if (FastDeepCloner.DeepCloner.GetFastDeepClonerProperties(item.GetType()).Any(x => x.GetCustomAttribute<ForeignKey>()?.Type == o.GetType()))
                            {
                                FastDeepCloner.DeepCloner.GetFastDeepClonerProperties(item.GetType()).First(x => x.GetCustomAttribute<ForeignKey>()?.Type == o.GetType()).SetValue(item, value);
                            }
                    ...
                    {
                        if (FastDeepCloner.DeepCloner.GetFastDeepClonerProperties(oValue.GetType()).Any(x => x.GetCustomAttribute<ForeignKey>()?.Type == o.GetType()))
                        {
                            FastDeepCloner.DeepCloner.GetFastDeepClonerProperties(oValue.GetType()).First(x => x.GetCustomAttribute<ForeignKey>()?.Type == o.GetType()).SetValue(oValue, value);
                        }
        ...
        internal static List<string> DeleteAbstract(ICustomRepository repository, object o, bool save = false)
        {
            ...
            foreach (var prop in props.Where(x => x.PropertyType.IsClass && x.PropertyType != typeof(string) && x.GetCustomAttribute<IndependentData>() == null && x.GetCustomAttribute<ExcludeFromAbstract>() == null))
            {
