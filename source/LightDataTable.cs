    ...
    public class LightDataTable : LightDataTableShared, ILightDataTable
    {
        ...
        public LightDataTable(object obj, bool execludeClasses = false, Type overdedDataType = null, bool ignoreTypeValidation = false) : base()
        {
            ...
            else if (obj is IList)
            {
                ...
                var T = FormatterServices.GetUninitializedObject(oList.GetType().GetProperties().ToList()[oList.GetType().GetProperties().Length - 1].PropertyType);
                foreach (var item in T.GetType().GetProperties())
                    if (!execludeClasses || (!item.PropertyType.IsClass || item.PropertyType == typeof(string)))
