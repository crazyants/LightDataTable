...
namespace Generic.LightDataTable
{
    public class LightDataTableRow : LightDataTableShared
    {
        ...
        public T TValueAndConvert<T>(string key, bool loadDefault = false)
        {
            ...
            var type = typeof(T);
            if (v != DBNull.Value && v != null && v.GetType() != Columns[key].DataType && !(type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>)) && v != "")
            ...
            if (v == null || v == DBNull.Value || string.IsNullOrEmpty(v.ToString()))
            {
                ...
                if (v == null && type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>))
                    return default(T);
            }
            else
                TypeValidation(ref v, type, false);

            if (v?.GetType() != type && (!type.IsGenericType || type.GetGenericTypeDefinition() != typeof(Nullable<>)))
        ...
        public LightDataTableRow MergeUnKnownObject(object objectToBeMerged)
        {
            if (!(objectToBeMerged is ExpandoObject))
            {
                ...
                {
                    var name = Columns.ContainsKey(property.Name) ? property.Name : MethodHelper.GetPropertyName(property);
        ...
        public void MergeToAnObject(object selectedObject)
        {
            ...
            {
                var name = Columns.ContainsKey(prop.Name) ? prop.Name : MethodHelper.GetPropertyName(prop);
        ...
        public T ToObject<T>()
        {
            ...
            var obj = o is IList
                ? 
                    o.GetType().GetProperties()[o.GetType().GetProperties().Length - 1].PropertyType.CreateInstance()
                : typeof(T).CreateInstance();
            foreach (var pr in FastDeepCloner.DeepCloner.GetFastDeepClonerProperties(obj.GetType()))
            {
                var name = MethodHelper.GetPropertyName(pr);
        ...
        public object ToObject(object t)
        {
            ...
            var obj = type.IsGenericType && type.GetGenericTypeDefinition().Name == "List`1" && type.GenericTypeArguments.Length > 0 ? type.GenericTypeArguments[0].CreateInstance() : type.CreateInstance();
